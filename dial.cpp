...
void Dial::setup(int analog_pin, int neopixel_pin, int first_pixel, int last_pixel, int dial_min, int dial_max)
{
  ...
  _dial_min = dial_min;
  _dial_max = dial_max;
  Adafruit_NeoPixel pixel(_last_pixel, neopixel_pin, NEO_RBG + NEO_KHZ800); //TODO edit which strip.
  pixels = pixel;
  pixels.begin();
}
...
void Dial::idle(void)
{
  ...
  if (dialPercent > _power_available){
    ...
    power_shortage = dialPercent - _power_available;
  }
  if (dialPercent <= _power_available){
    powerProduced = dialPercent;
    power_unused = _power_available - dialPercent;
  }
  ...
  for(int i=_first_pixel; i<_last_pixel; i++) { // For each pixel...
    if (i <= pp) pixels.setPixelColor(i, pixels.Color(50, 0, 0));
    if (i > pp && i<= pu) pixels.setPixelColor(i, pixels.Color(0, 20, 0));
    if (i > pp && i<= ps) pixels.setPixelColor(i, pixels.Color(0, 0, 20));
  }
  pixels.begin();
...
int Dial::getDialPercentage(void)
{
